...
app.post("/api/explain-verse", async (req, res): Promise<any> => {
  const { verse, reference, grade } = req.body;
  if (!verse || !reference) {
...
1. **쉽게 이해하기**: 이 말씀은 어떤 상황 속에서도 하나님께서 우리 손을 꼭 잡아 주시고, 지혜를 주셔서 어려운 시험이나 근심도 모두 용기 백배 이겨낼 수 있도록 도우신다는 사랑의 말씀이에요.
2. **세 줄 약속 실천 행동 가이드**:
   - 매일 아침 눈뜰 때 "하나님 오늘 하루도 감사의 고백을 들려드릴게요"라고 속삭여 봐요.
   - 공부방 앞에 이 요절을 귀엽게 적어 붙여두고 틈틈이 소리 내어 보아요.
   - 오늘 하루 내 가장 소중한 일기장이나 수첩에 이 구절을 정성스레 적어 보아요.

*💖 격려의 선물: "말씀을 보배처럼 귀하게 마음에 보관하는 우리 친구는 장차 주님 앞에 온전히 쓰임 받는 든든한 일등 공신이 될 거예요!"*`;
...
사용자가 지정한 대상학년/난이도 [${gradeText}] 및 성경 범위 [${topic}]에 맞춰서 어린이들이 재미있게 공부할 수 있는 최고급 요약노트, 해당 범위의 흐름을 상세하게 요약 정리해 둔 6단계 수준의 소제목스토리(detailedStories) 타임라인, 핵심 요절 암송 구절 정확히 10개 (중요 단어에 [ ] 대괄호가 쳐진 빈칸 문제와 그 단어 목록 배열 포함), 대표 인물 프로필 1개, 그리고 선생님이 어린이를 일대일 구도/구두로 질문하여 직접 대답을 훈련할 수 있는 "교사업무 피드백용 대화형 Q&A 단답형 퀴즈 세트" 5문항을 만드십시오.
